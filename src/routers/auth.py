...
# Авторизация
@router.post('/auth')
async def auth_user(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(dependencies.get_db)):
...
@router.get('/users/me')
async def read_users_me(token: str = Depends(oauth2_scheme), db: Session = Depends(dependencies.get_db)):
